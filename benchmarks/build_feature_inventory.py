...
feature('native.cuda_fused','자체 실행·출력','Fused CUDA Yee/CPML kernel','implemented','implemented','n/a',
        '선택형 실수 CUDA kernel. 2D/3D, float32/64, CPML·periodic·graded 및 기존 ADE. 별도 선택형 plane DFT는 위상·공간 보간·복소 누적을 single/batch에서 공유 launch. Python cuda_graph_steps는 관측 시점을 보존한 선택형 graph unrolling. 5880 장·스펙트럼 비교와 준비 비용·회귀 포함 성능 측정. Bloch는 torch 경로. Adjoint는 미구현.',
...
updates={
 ...
 'workflow.fsp_write':('partial','partial','독립 기존 primitive geometry writeback. 위치·크기·회전·가변 polygon·이름·상수 재료·priority. 원본 fingerprint와 미수정 byte 보존, Python/CLI/UI. 소스 시간/위상·모니터 주파수/window·duration/CFL·일부 PML/Periodic와 uniform target 메시 간격/크기 writeback. 입력 controls·node·PML 범위·dt 갱신 및 native E/H/DFT 왕복 검사. Primitive 추가·삭제·복제·순서 저장과 ID/미수정 byte 대응표 추가. 새 레코드는 자체 metadata 기본값을 쓰며 외부 reader 미검증. 외부 remeshing·전체 저장·소스/모니터 목록·일반 설정·그룹·결과는 남음.',('photonweave/fsp_geometry.py','photonweave/fsp_settings.py','tests/test_fsp_geometry_write.py','tests/test_fsp_settings_write.py','tests/test_fsp_mesh_write.py','tests/ui/fsp-settings-write.spec.js','tests/ui/fsp-mesh-write.spec.js','photonweave/fsp_objects.py','tests/test_fsp_objects_write.py','tests/ui/fsp-objects-write.spec.js')),
 ...
 'material.multipole':('implemented','implemented','최대 16개 수동 Drude/Lorentz pole의 합. 공통 E를 동시에 구하는 trapezoidal ADE, n/k preview·JSON/Python·GUI pole 편집. 측정 데이터 피팅은 별도 미구현.',('photonweave/materials.py','tests/test_multipole.py','frontend/src/materials.js')),
