...
updates={
 ...
 'workflow.optimization':('partial','missing','Python 병렬 DE와 별도 Torch API의 Adam 예제. 고정 Bloch를 포함한 epsilon·점/스펙트럼/고정 plane 목적함수, 일부 형상 graph와 resident/streamed ADE 재료 gradient 지원. 일반 소자 최적화·물리 shape-gradient 수렴·gradient microbatch·UI는 남음.',('photonweave/design.py','photonweave/differentiable.py','photonweave/dispersive_adjoint.py','examples/differentiable_design.py','examples/differentiable_dispersive_design.py','tests/test_differentiable.py','docs/CR_VALIDATION_PLAN.md')),
...
for row in features:
    ...
    if row['id'] == 'source.vector':
        row['fsp'] = 'partial'
feature('workflow.adjoint','실행·자동화·호환','Adjoint·자동미분 FDTD','partial','missing','n/a',scope='고정 실수/복소 Bloch의 diagonal epsilon→Yee/CPML→점·주파수·고정 plane 관측→Torch backward. Fused transpose·제한 checkpoint·공간 slab·형상 graph와 ADE 미분 지원. 공유 메모리 예산의 순차 case replay로 서로 다른 격자·관측·재료를 하나의 목적함수에 연결. PeriodicLayerResponse가 밀도→편광 합성→검출 응답을 같은 예산의 resident/DRAM/파일 실행에 연결하며 source graph를 하나씩 재계산. 512³ resident 및 54GiB 상태의 짧은 용량/VJP 검증 완료. 장시간 응용, 동시 adjoint microbatch, TFSF·mode port·coupled subpixel 및 물리 shape-gradient 수렴은 남음.',evidence=('photonweave/differentiable.py','tests/test_differentiable.py','photonweave/dispersive_adjoint.py','tests/test_dispersive_adjoint.py','photonweave/streamed_dispersive.py','tests/test_streamed_dispersive.py','photonweave/adjoint_batch.py','tests/test_adjoint_batch.py','docs/DIFFERENTIABLE_FDTD.md','docs/DISPERSIVE_ADJOINT.md','docs/ADJOINT_BATCH.md','photonweave/periodic_adjoint.py','tests/test_periodic_adjoint.py','docs/PERIODIC_HIERARCHICAL_DESIGN.md'),reference='',priority=0)
feature('workflow.memory_hierarchy','실행·자동화·호환','GPU·DRAM·디스크 계층형 checkpoint','partial','missing','n/a',scope='명시적인 tier별 slot, binomial 재계산, 무손실 state 저장과 byte 한도, 비동기 checkpoint staging 및 혼합 tier 구현. 별도 slab API에서 DRAM/파일 공간 backing·비동기 전송 지원. Resident/streamed 후보의 실측 선택과 여러 case의 출력·gradient·활성 solver 공동 예산 추가. 전체 host RSS·OS cache 한도, 장시간 응용에서의 성능 선택 검증과 UI는 미완료.',evidence=('photonweave/differentiable.py','tests/test_differentiable.py','photonweave/streamed.py','photonweave/execution_tuning.py','tests/test_execution_tuning.py','photonweave/adjoint_batch.py','docs/HIERARCHICAL_EXECUTION.md','docs/EXECUTION_SELECTION.md','docs/ADJOINT_BATCH.md'),reference='',priority=0)
feature('workflow.out_of_core','실행·자동화·호환','Space-time tiled out-of-core GPU FDTD','partial','missing','n/a',scope='실험적 실수/복소 Bloch slab·causal halo transpose·DRAM/파일 backing·비동기 staging·tile/K 실측 tuning 구현. 54GiB E/H와 별도 54GiB E/H+ADE P/Q의 10-step forward/gradient 검증 완료. 점 스펙트럼·고정 검출면의 resident/streamed 통합 선택과 기준 gradient 캐시 한도 지원. 용량과 속도는 별도이며 장시간 응용·전체 메모리 계측·동시 microbatch 공동 튜닝·UI는 미완료.',evidence=('photonweave/streamed.py','photonweave/streamed_dispersive.py','tests/test_streamed_dispersive.py','photonweave/spacetime.py','photonweave/tile_workspace.py','tests/test_spacetime.py','tests/test_complex_spacetime_cuda.py','photonweave/plane_execution.py','tests/test_plane_execution.py','docs/STREAMED_FDTD.md','docs/BEYOND_VRAM_VALIDATION.md','docs/EXECUTION_SELECTION.md'),reference='',priority=0)
