...
for row in features:
    ...
    if row['id'] == 'source.vector':
        row['fsp'] = 'partial'
feature('workflow.adjoint','실행·자동화·호환','Adjoint·자동미분 FDTD','partial','missing','n/a',scope='고정 실수/복소 Bloch의 diagonal epsilon→Yee/CPML→점·주파수·고정 plane 관측→Torch backward. Fused transpose·제한 checkpoint·공간 slab·형상 graph 지원. 명시적 ADE 재료 미분은 resident Torch 및 fused CUDA 경로. 공간 ADE, TFSF·mode port·coupled subpixel 및 물리 shape-gradient 수렴은 남음.',evidence=('photonweave/differentiable.py','tests/test_differentiable.py','photonweave/dispersive_adjoint.py','tests/test_dispersive_adjoint.py','docs/DIFFERENTIABLE_FDTD.md','docs/DISPERSIVE_ADJOINT.md'),reference='',priority=0)
...
feature('workflow.out_of_core','실행·자동화·호환','Space-time tiled out-of-core GPU FDTD','partial','missing','n/a',scope='실험적 비분산 실수/복소 Bloch slab·causal halo transpose·DRAM/파일 backing·비동기 staging·tile/K 실측 tuning 구현. 54GiB E/H의 10-step forward/gradient 검증 완료. 장시간 응용 속도·전체 메모리 계측·공간 ADE·통합 자동 정책·UI는 미완료.',evidence=('photonweave/streamed.py','photonweave/spacetime.py','photonweave/tile_workspace.py','tests/test_spacetime.py','tests/test_complex_spacetime_cuda.py','docs/STREAMED_FDTD.md','docs/BEYOND_VRAM_VALIDATION.md'),reference='',priority=0)
