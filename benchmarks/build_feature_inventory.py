...
for row in features:
    ...
    if row['id'] == 'source.vector':
        row['fsp'] = 'partial'
feature('workflow.adjoint','실행·자동화·호환','Adjoint·자동미분 FDTD','partial','missing','n/a',scope='고정 실수/복소 Bloch의 diagonal epsilon→Yee/CPML→점·주파수·고정 plane 관측→Torch backward. Fused transpose·제한 checkpoint·공간 slab·형상 graph 지원. 명시적 ADE 미분은 resident Torch·fused CUDA 및 실험적 DRAM/파일 공간 타일 경로. 분산 대규모 검증, TFSF·mode port·coupled subpixel 및 물리 shape-gradient 수렴은 남음.',evidence=('photonweave/differentiable.py','tests/test_differentiable.py','photonweave/dispersive_adjoint.py','tests/test_dispersive_adjoint.py','photonweave/streamed_dispersive.py','tests/test_streamed_dispersive.py','docs/DIFFERENTIABLE_FDTD.md','docs/DISPERSIVE_ADJOINT.md'),reference='',priority=0)
feature('workflow.memory_hierarchy','실행·자동화·호환','GPU·DRAM·디스크 계층형 checkpoint','partial','missing','n/a',scope='명시적인 tier별 slot, binomial 재계산, 무손실 state 저장과 byte 한도, 비동기 checkpoint staging 및 혼합 tier 구현. 별도 slab API에서 DRAM/파일 공간 backing·비동기 전송 지원. 전체 host RSS·OS cache 한도와 resident까지 통합한 성능 정책은 미완료.',evidence=('photonweave/differentiable.py','tests/test_differentiable.py','photonweave/streamed.py','docs/HIERARCHICAL_EXECUTION.md'),reference='',priority=0)
feature('workflow.out_of_core','실행·자동화·호환','Space-time tiled out-of-core GPU FDTD','partial','missing','n/a',scope='실험적 실수/복소 Bloch slab·causal halo transpose·DRAM/파일 backing·비동기 staging·tile/K 실측 tuning 구현. 54GiB E/H의 10-step forward/gradient 검증 완료. ADE P/Q 공간 저장·재료 gradient, 분산 비용 측정·기본 타일 예산 조정·기준 gradient 캐시 한도를 추가. 분산 대규모 용량·속도, 장시간 응용·전체 메모리 계측·통합 자동 정책·UI는 미완료.',evidence=('photonweave/streamed.py','photonweave/streamed_dispersive.py','tests/test_streamed_dispersive.py','photonweave/spacetime.py','photonweave/tile_workspace.py','tests/test_spacetime.py','tests/test_complex_spacetime_cuda.py','docs/STREAMED_FDTD.md','docs/BEYOND_VRAM_VALIDATION.md'),reference='',priority=0)
