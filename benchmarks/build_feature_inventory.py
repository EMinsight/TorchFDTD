...
feature('workflow.out_of_core','실행·자동화·호환','Space-time tiled out-of-core GPU FDTD','partial','missing','n/a',scope='실험적 실수/복소 Bloch slab·causal halo transpose·DRAM/파일 backing·비동기 staging·tile/K 실측 tuning 구현. 54GiB E/H의 10-step forward/gradient 검증 완료. ADE P/Q 공간 저장·재료 gradient를 추가. 분산 대규모 용량·속도, 장시간 응용·전체 메모리 계측·통합 자동 정책·UI는 미완료.',evidence=('photonweave/streamed.py','photonweave/streamed_dispersive.py','tests/test_streamed_dispersive.py','photonweave/spacetime.py','photonweave/tile_workspace.py','tests/test_spacetime.py','tests/test_complex_spacetime_cuda.py','docs/STREAMED_FDTD.md','docs/BEYOND_VRAM_VALIDATION.md'),reference='',priority=0)
