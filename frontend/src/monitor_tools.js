 ...
 return {
  ...
  async flux(){
   ...
   async function plot(){const token=++revision;try{
    ...
    if(ref){const result=await api(`/jobs/${state.job}/normalize-flux?reference=${encodeURIComponent(ref)}&monitor=${encodeURIComponent(m.id)}&subtract_incident=${subtract}`);if(token!==revision)return;series={...m,...result,spectrum:result.ratio,signed:true,spectrum_label:'Wavelength (µm) · signed normalized flux'};const reasons=[...new Set(result.reasons.filter(Boolean))];$('.monitor-status').textContent=`${result.valid.filter(Boolean).length}/${result.valid.length} frequencies above the reference threshold${reasons.length?` (invalid entries: ${reasons.join('; ')})`:''}. Reflection is negative for propagation opposite the positive normal.`;}
