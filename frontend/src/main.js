...
function executionStatus(e){
 ...
 if(!e.mode)return `<span class="warning error">${esc(prefix+(e.error||'no execution mode fits'))}</span>`;
 const warnings=(e.warnings||[]).map(w=>`<span class="warning">${esc(w)}</span>`).join('');
 return `<b>${esc(prefix+modeLabels[e.mode])}</b> on ${e.backend==='cuda'?'GPU':'CPU'} · ${esc(e.reason||'')}${e.mode==='streamed_disk'?` · scratch ${esc(e.scratch_directory)}`:''}${warnings}`;
}
function tiledControls(r){
 if(r.execution_mode!=='tiled')return '';
 r.tiling??={size_um:20,overlap_um:2,max_angle_deg:45,propagation_um:null};const t=r.tiling,plan=stats?.execution?.tiled?.plan,s=plan?.suggestion;
...
function renderResults(){const r=state.project.region,axis='xyz'.indexOf(r.slice_axis);let dims=(state.results?.summary?.actual_size_um||r.size).filter((_,i)=>i!==axis);const result=state.results,frame=result?.frames[state.frame]||state.liveFrame,tiled=result?.summary?.execution?.frames?.[state.frame];
 const stale=$('#field-view').classList.contains('stale');$('#field-label').textContent=(stale?'STALE · ':'')+(tiled?`${tiled.label} · ${tiled.axes.join('').toUpperCase()} plane`:r.field+' · '+r.complex_display+' · '+['YZ','XZ','XY'][axis]+' plane');$('#frame-label').textContent=tiled?tiled.plane+' plane':result?(result.frames.length?'Step '+result.frame_steps[state.frame]:'No field snapshot'):'Live field';if(tiled)dims=tiled.span_um;$('#frame-slider').max=Math.max(0,(result?.frames.length||1)-1);$('#frame-slider').value=state.frame;
...
const actions={
 ...
 delete:()=>{if(state.mode!=='layout'||!selection().length)return;remember();const ids=new Set(selection());for(const cat of ['structures','sources','monitors'])state.project[cat]=state.project[cat].filter(o=>!ids.has(o.id));persist();select('fdtd');validate();log(`Deleted ${ids.size} object${ids.size===1?'':'s'}.`);},
