...
function renderProperties(){
 ...
 html+=section('General',dropdown('dimension','dimension',r.dimension,[['2d','2D (XY)'],['3d','3D']])+dropdown('resource','backend',r.backend,[['auto','GPU if available'],['cuda','GPU · CUDA'],['cpu','CPU']])+dropdown('precision','precision',r.precision,['float32','float64'])+dropdown('CUDA kernel','cuda_kernel',r.cuda_kernel||'torch',[['torch','PyTorch reference'],['fused','Fused Yee / CPML (experimental)']])+dropdown('Frequency monitor kernel','cuda_monitor_kernel',r.cuda_monitor_kernel||'torch',[['torch','PyTorch reference'],['fused','Shared CUDA plane DFT (experimental)']]));
...
async function validate(){try{stats=await api('/validate',state.project);$('#mesh-summary').innerHTML=`<strong>${stats.shape.join(' × ')}</strong><span>${stats.cells.toLocaleString()} cells · ~${Number(stats.estimated_memory_mb).toFixed(1)} MB</span>${stats.mesh_type==='graded'?`<span>${stats.cell_reduction_percent.toFixed(1)}% fewer cells than uniform</span>`:''}<span>Δt ${stats.dt_fs.toFixed(4)} fs · ${stats.duration_fs.toFixed(1)} fs total</span>${stats.warnings.map(w=>`<p class="warning">${esc(w)}</p>`).join('')}`;$('#footer-grid').textContent=stats.shape.join(' × ')+' cells';return true;}catch(e){$('#mesh-summary').innerHTML=`<p class="warning error">${esc(e.message)}</p>`;return false;}}
