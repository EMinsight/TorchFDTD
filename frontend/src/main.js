...
function renderResults(){const r=state.project.region,axis='xyz'.indexOf(r.slice_axis);let dims=(state.results?.summary?.actual_size_um||r.size).filter((_,i)=>i!==axis);const result=state.results,frame=result?.frames[state.frame]||state.liveFrame,tiled=result?.summary?.execution?.frames?.[state.frame];
 $('#field-label').textContent=tiled?`${tiled.label} · ${tiled.axes.join('').toUpperCase()} plane`:r.field+' · '+r.complex_display+' · '+['YZ','XZ','XY'][axis]+' plane';$('#frame-label').textContent=tiled?tiled.plane+' plane':result?(result.frames.length?'Step '+result.frame_steps[state.frame]:'No field snapshot'):'Live field';if(tiled)dims=tiled.span_um;$('#frame-slider').max=Math.max(0,(result?.frames.length||1)-1);$('#frame-slider').value=state.frame;
