...
def test_parallel_serial_agreement_resume_and_safe_paths(tmp_path):
    ...
    with BatchRunner(backend='cpu',max_workers=2) as runner:
        report=runner.run(cases,objective=trace_energy,objective_key='energy-v1',output_dir=tmp_path,keep_results=True)
        report.raise_for_errors()
        assert len({item.pid for item in report.items})==2
        # Two independent processes actually execute simulations concurrently.
