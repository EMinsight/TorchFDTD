...
SOLID_FORWARD_SHA = '2ebd56e8ceb6fcf09a59a46b64b14afdd90a618c37332027278737bb5217cfe6'
SOLID_GRADIENT = [8.872056388275427, 66.33811558833472, -78.66300315125868, -0.08282598286201565,
                  -0.37789172133901927, -0.8303187137259035, -2.8946998305268195, 169.00180759381277, 99.30053372511483]
SOLID_BACKGROUND_GRADIENT = -14.47232522153175
DENSITY_FORWARD_SHA = '023bb2d564b507301e3e7e4bec598942c6781236835cf7afa5607fcb404a4f2d'
...
def test_existing_solid_and_density_gradients_are_unchanged():
    ...
    assert hashlib.sha256(epsilon.detach().numpy().tobytes()).hexdigest() == SOLID_FORWARD_SHA
    assert dp.tolist() == SOLID_GRADIENT and float(db) == SOLID_BACKGROUND_GRADIENT
    ...
    assert hashlib.sha256(density.detach().numpy().tobytes()).hexdigest() == DENSITY_FORWARD_SHA
    assert model.design.grad.flatten().tolist() == DENSITY_GRADIENT
