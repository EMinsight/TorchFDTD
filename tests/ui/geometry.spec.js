...
test('polygon editing, ordered rotations, ellipse controls and GPU solve',async({page})=>{
 ...
 const before=await page.evaluate(()=>JSON.parse(localStorage.getItem('photonweave.project.v1')).structures.at(-1).vertices);
...
test('synthetic ellipse FSP to editable native scene preserves input bytes',async({page})=>{
 ...
 await expect.poll(()=>page.evaluate(()=>JSON.parse(localStorage.getItem('photonweave.project.v1'))?.structures?.[0]?.make_ellipsoid)).toBe(true);
 const p=await page.evaluate(()=>JSON.parse(localStorage.getItem('photonweave.project.v1')));expect(p.structures[0].make_ellipsoid).toBe(true);
