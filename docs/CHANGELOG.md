...
- The 2D waveguide and cylinder examples launch a one-way sheet toward +x (periodic in y, the waveguide starting at x = -2 and excited end-fire) instead of soft sources whose backward half ran into the left PML; the layout views draw a propagation arrow on every plane source, one way for one-way planes and both ways for soft sheets (this commit).
