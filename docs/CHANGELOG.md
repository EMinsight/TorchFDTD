...
- `torchfdtd serve --memory-admission` and `create_app(memory_admission=True)`: the workbench server admits scenes by the memory estimate and the caps a project carries, as the Python API does, instead of `SERVER_LIMITS`, in its requests, job threads and modal worker; a resident run also has its host outputs (final E and H copies, point traces, display frames) admitted against 80% of the available host memory. The input limits (request and upload sizes, FSP and GDS parser limits, text caps, route bounds, one million cells per axis, the 2^31 index bound) and the loopback, `Host` and `Origin` checks stay. `/api/health` reports `admission` and `server_limits`, and the execution panel shows the admission. The default is unchanged; the mode is meant for a single user on their own machine ([SECURITY.md](SECURITY.md#memory-admission), this commit).
