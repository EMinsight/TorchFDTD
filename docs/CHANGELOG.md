...
- `scripts/rerecord_gates.py` re-records every replayable gate task on one commit from its newest evidence (command, environment prefix, case file, scope) and, with `--wheel`, with the installed wheel from a fresh `.local/venvs/rc` environment; `scripts/record_gate_evidence.py --interpreter` records the environment of that interpreter. The release-candidate run is written down in [RELEASE_PROCEDURE.md](RELEASE_PROCEDURE.md) (this commit).
