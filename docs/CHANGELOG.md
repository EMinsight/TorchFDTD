...
- The workbench server checks, in project validation and from counts alone, the host memory that validation, `estimate()` and `resolve_plan` will take (`torchfdtd.solver.preadmission_bytes`: steps, source terms, monitor frequencies, plane points, Bloch sheet cells and list items) against 80% of the available host memory before any array is built; 10**12 steps, frequencies or plane points answer 422 in either mode. `valid_scene` checks the temporal Nyquist limit and `estimate()` counts frequencies and plane points without building them. The source preview covers the first 100,000 steps of a longer run (`preview_steps`), `GET /api/jobs/{key}` sends point-monitor series strided to at most 400,000 values, a finished job keeps its spectra strided to at most 4,000,000 values for `spectra.csv`, and `GET /api/jobs/{key}/field-monitors/{id}` sends a plane strided to at most 512 x 512 points (`full_shape`, `stride`); the NPZ download keeps every array (this commit).
