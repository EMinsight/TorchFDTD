...
\subsection{Meep and FDTDX on one workstation}

The comparisons above use the package's own baselines. Tables~\ref{tab:cross-solver-accuracy} and~\ref{tab:cross-solver-speed} place TorchFDTD next to Meep \citep{oskooi2010} and FDTDX \citep{mahlau2024} on one workstation with an RTX 3060 and a twelve-core processor, on fixtures that are defined once and instantiated by one driver per solver with the same mesh, time step, absorber thickness, source waveform and duration: the analytic slab and the Mie sphere of Section~\ref{sec:validation}, the vacuum and sphere scenes of Table~\ref{tab:open-source} at $64^{3}$ and $96^{3}$ cells, and a periodic full-material-gradient adjoint. TorchFDTD and FDTDX receive the same sampled permittivity arrays and the same additive source samples, and their point traces and material gradients agree to the relative $L_2$ differences listed in the record. Meep runs in double precision on twelve MPI ranks with its own absorber and current-source conventions, offers no closed TFSF box, and its frequency-domain adjoint is not comparable, so it is absent from the adjoint row. FDTDX 0.6.2 offers no closed TFSF box and no two-dimensional mode either. The records, the solver-by-solver differences and the rerun commands are in docs/CROSS\_SOLVER\_COMPARISON.md.
...
We demonstrate a time-domain solver in which the same fused CUDA kernels serve the forward run, its transposed sweep and a streamed execution whose field state may exceed the processor by more than an order of magnitude, with a device footprint of 2 to 3 GB for a 54 GB problem. The measured reservations and the journal turn a large adjoint from a single fragile allocation into a sequence of block records that can be planned, budgeted and resumed.
