...
\title{Hierarchical discrete adjoints in TorchFDTD\\\large Development methods supplement}
\author{Hyoseok Park}
...
A completed $256^3$, 128-step dielectric measurement on RTX 5880 gives
medians of 448.5862 s and 407.6886 s for six and twelve CPU threads.
Resident CUDA takes 2.2834 s and asynchronous DRAM slabs take 9.4349 s,
corresponding to ratios of 178.55 and 43.21 against the faster tested CPU
median. All 77 source hashes are verified. Measured output and material-VJP
relative errors remain below $2.41\times10^{-7}$. This problem fits VRAM.
The two-pole dispersive measurement is still pending.
...
\section{Restartable full-schedule density optimization}
An explicit-input driver now maximizes the fixed-calibration spectral electron
information objective through periodic density responses and projected Adam.
Every update uses all supplied wavelength and pupil-ray cases. Bounded case
replay retains one optical solver graph at a time, and the optimizer works on
the CPU density tensor. A conservative design and Adam copy allowance is
subtracted from the shared solver host budget. Context and runtime overhead
remain outside this tensor reservation.

An atomically replaced checkpoint retains density, Adam moments, step count,
compact objective history and the best evaluated design. A run owns an OS file
lock and checks exact input, source, execution-policy and runtime identities
when resuming. Completed forward cases can be restored, while their gradient
replay executes the differentiable solver again. An uncommitted optimizer
update is discarded after interruption. The final updated density receives
a complete forward objective before export. This is projected continuous
density optimization, without a new optimizer algorithm, binarization or
an implicit claim of physical CR convergence.

A synthetic three-wavelength, one-ray check with a $2\times2$ density increases
information from 0.07000366558 to 0.07002771570 bits per pixel over two resident
updates. Restarting after the first update preserves the final response and
density exactly. One asynchronous DRAM update agrees with its resident
counterpart. These checks establish discrete optimizer connectivity and
restart behavior, not performance or a completed original CR application.

\section{Default single-precision execution}
The periodic density API and complete CR evaluation and optimization drivers
now default to FP32. Density, optical response, information matrices, density
gradients and Adam moments share this precision. Bloch optical fields use
complex64. FP64 is an explicit validation mode, rather than a mandatory
stage in each objective evaluation. Imported fixed calibration matrices are
converted to the selected response precision. Restart contracts bind that
selection, so a checkpoint cannot silently switch precision.

FP32 and FP64 optimizer tests cover uninterrupted and resumed runs, bounded
case replay and agreement between resident and asynchronous DRAM execution.
These checks establish discrete connectivity and restart behavior. They do
not establish precision independence for arbitrary devices or physical CR
gradient convergence. Subsequent beyond-VRAM capacity studies should exceed
the available VRAM at FP32, with state size reported separately from transient
and checkpoint allocations.

A complete 144-case application comparison now evaluates nine wavelengths
and sixteen pupil rays for the original binary seed at a 50 nm mesh and
1,600 time steps. The FP32 RTX 3060 response and full density VJP differ from
the archived FP64 RTX 5880 reference by relative Euclidean errors of
$5.04\times10^{-7}$ and $1.75\times10^{-5}$. The information difference is
$5.05\times10^{-6}$ bits per pixel. All meet the predeclared comparison
limits. The archived reference already passed a directional derivative
check. This result verifies this discrete application across precision and
hardware, without establishing physical gradient convergence, completed
optimization or hardware speedup.

\section{Capacity-first periodic execution admission}
The periodic response API now offers selection without calibration solves.
It first attempts resident admission under shared GPU and host budgets.
If rejected, it decreases slab width and temporal depth until asynchronous
DRAM execution fits. Explicitly configured file banks form the final tier,
with a separate free-space reserve. Geometry, precision, physical duration
and checkpoint count remain fixed. The selected policy is reused for the
forward and backward calls, and live execution admission is retained.
This is a capacity heuristic rather than a measured performance optimum.

The completed complex-FP64 dense-plane study illustrates why calibration is
not mandatory for ordinary execution. With nine policies and a 256-step
held-out duration, the fastest resident median is 11.8467 s, while calibration
costs 399.602 s. The initially admitted resident baseline is already fastest.
Asynchronous width-32, depth-eight streaming takes 56.0846 s, compared with
91.4281 s synchronously. All full outputs and material VJPs pass comparison
with the recorded resident reference. These measurements fit VRAM and do not
establish a beyond-VRAM speed advantage.

\section{Bounded replay of parameterized geometry}
An additional geometry layer maps tensor-valued dimensions, centers, rotations
and material values to diagonal permittivity at the fixed electric sample
locations. Boxes, ellipsoids and finite cylinders use compact cubic occupancy
transitions. For an occupancy $a_s$ and solid permittivity $\epsilon_s^{\rm solid}$,
ordered composition is
\begin{equation}
\epsilon_s=(1-a_s)\epsilon_{s-1}+a_s\epsilon_s^{\rm solid}.
\end{equation}
This defines an explicitly regularized geometry and overlap order. It does not
implement the coupled sharp-interface subpixel operator.

The geometry backward retains parameters and one-dimensional coordinate axes.
For each spatial chunk $c$, it regenerates the local geometry graph and applies
the material VJP returned by the Maxwell adjoint:
\begin{equation}
\nabla_{\boldsymbol\theta}L=
\sum_c\left(\frac{\partial\epsilon_c}{\partial\boldsymbol\theta}\right)^T
\nabla_{\epsilon_c}L.
\end{equation}
With $N$ cells, $S$ solids and $C$ cells per chunk, the retained local geometry
graph requires $O(CS)$ storage rather than $O(NS)$ intermediate arrays. Dense
permittivity and incoming material gradients still require $O(N)$ storage.
First-order CPU and CUDA chains are checked against full-time autograd and
spatial replay in FP32. A separate directional finite-difference check uses
FP64. These checks establish the discrete regularized chain, not physical
shape-gradient convergence or a new sharp-interface derivative algorithm.

\section{Material production inside spatial replay}
A second API eliminates the dense permittivity and material-gradient arrays
for the supported analytic solids. Let $P_i$ list the primary-domain sample
indices of tile $i$, including repeated periodic halo indices. The tile
material is generated as $\epsilon_i=G(P_i,\boldsymbol\theta)$ on the host
immediately before staging. Material values do not acquire a Bloch phase.
During reverse block propagation the material seed is reduced directly:
\begin{equation}
\overline{\boldsymbol\theta}=
\sum_{b,i}
\left[D_{\boldsymbol\theta}G(P_{b,i},\boldsymbol\theta)\right]^T
\overline{\epsilon}_{b,i}.
\end{equation}
Here $b$ indexes differentiated time blocks. Each tile returns only its owned
physical outputs, while every halo material contribution is included in the
parameter reduction. This is the chain rule for the same spatial replay
operator, without first forming a global $\overline\epsilon$.

Material workspace scales with the extended tile, the bounded geometry chunk,
the solid parameter packet and the one-dimensional axes. Field and CPML banks
still scale with the full domain and reside in host memory or files.
A private coordinate snapshot fixes the replay geometry after forward.
Memory admission precedes that snapshot and plane-layout allocation and
includes their storage. Fixed collocated planes retain the existing flux
and matched-reference normalization derivatives. Tests compare host and
file execution, periodic halo replication, complex Bloch spectra and
asynchronous CUDA tiles with the dense material path. This API does not
yet route automatic CR execution through the producer. Dispersive geometry,
one-way material validation and TFSF adjoints remain outside its scope.
No large-domain speed or capacity result is inferred from the small tests.

\section{Periodic-density material streaming}
An extruded periodic design has a two-dimensional density matrix $D$.
For electric component $c$, let $W_{x,c}$ and $W_{y,c}$ contain periodic
pixel-overlap weights at that component's Yee coordinates, and let $f_c(z)$
be its fixed layer-height overlap fraction. An extended slab has
\begin{equation}
 \epsilon_c(x,y,z)=\epsilon_b+
 (\epsilon_d-\epsilon_b)(W_{x,c}D W_{y,c}^{T})_{xy}f_c(z).
\end{equation}
The material transpose accumulates directly into the density,
\begin{equation}
 \overline D \mathrel{+}=
 (\epsilon_d-\epsilon_b)\sum_c W_{x,c}^{T}
 \left[\sum_z\overline\epsilon_c(x,y,z)f_c(z)\right] W_{y,c}.
\end{equation}
Repeated periodic halo rows retain their own contributions in this sum.
Material values carry no Bloch phase. The overlap matrices are rebuilt per
component and slab. No full three-dimensional material or material-VJP array
is retained. The producer and reduction run on CPU while field tiles may run
on CUDA. Homogeneous references use zero density. Source-basis calibration,
coherent polarization mixing and detector normalization remain unchanged.

The streamed periodic-response API uses this producer automatically for
explicit streamed policies. Two source-basis cases still execute sequentially
and replay for backward. Tests cover both density pixel origins, fractional
height interfaces, repeated seams, host and file state banks, asynchronous
CUDA, shared admission and full response gradients. These synthetic tests
do not establish workstation-scale throughput or physical CR convergence.
Interfaces and selected-frequency permittivities are fixed. One-way sources,
TFSF and dispersive density materials remain unsupported by this producer.

\section{Physical slab shape-gradient refinement}
An FP32 study uses an air--dielectric--air slab of thickness
$0.23\,\mu{\rm m}$ and relative permittivity 2.25 at wavelength
$1.55\,\mu{\rm m}$. The independent sharp-interface transmission is
\begin{equation}
T=\left[1+\frac{(\epsilon-1)^2}{4\epsilon}
\sin^2\left(\frac{2\pi\sqrt{\epsilon}d}{\lambda}\right)\right]^{-1}.
\end{equation}
The derivatives with respect to thickness and permittivity follow from this
expression. The physical domain, source, observation locations, 90 fs
duration and $0.4\,\mu{\rm m}$ PML thickness remain fixed during refinement.
Each grid uses a matched air reference.

\begin{center}
\begin{tabular}{rrrrr}
\toprule
Mesh & Width & $|T-T_{\rm sharp}|$ & Thickness error & Permittivity error\\
$(\mu{\rm m})$ & $(\mu{\rm m})$ & & (\%) & (\%)\\
\midrule
0.04 & 0.16 & 0.0152381 & 12.0901 & 11.9563\\
0.02 & 0.12 & 0.0089792 & 6.8438 & 7.0096\\
0.01 & 0.08 & 0.0040911 & 2.7977 & 3.1889\\
0.01 & 0.06 & 0.0022881 & 1.6326 & 1.7834\\
\bottomrule
\end{tabular}
\end{center}
The initial finest case fails the declared 3 percent derivative criterion.
The final row is an explicit regularization followup with the same criterion,
not a replacement of that failed record. Separate time and PML controls
change the final observables and derivatives by less than $1.8\times10^{-7}$.
Keeping a fixed transition width while refining the mesh leaves a nonzero
sharp-interface bias. Small thickness and permittivity moves selected by
the gradients increase both measured and analytic transmission.
The record \texttt{gradient\_mesh\_3060.json} retains the initial failure,
controls and provenance. These results concern a normally incident planar
slab, not arbitrary curved interfaces or the CR objective.
