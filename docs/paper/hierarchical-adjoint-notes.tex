...
device's 51,526,500,352 bytes of physical VRAM. Ten forward steps completed
in 741.239 seconds using file-backed spatial tiles. The maximum simultaneous
logical file storage was 117,012,692,992 bytes. Point histories agreed with
a smaller full-autograd domain whose boundaries lie outside the finite
dependency cone. This short experiment tests forward capacity and consistency.
It does not establish a converged optical response. Backward accuracy and
complete all-tier peak memory remain pending in this experiment.
The backward run was interrupted by a workstation restart attributed by the
Windows event log to a Start menu restart request. Its forward snapshot is
preserved, but no complete capacity-gradient result is available from that run.
\end{document}
