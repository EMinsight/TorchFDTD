...
only, make the support table of RELEASE_SCOPE.md come from the gate file
instead of hand-typed cells, and give the maintainer one tool that re-records
every gate on the exact release-candidate tree and wheel, with the procedure
written down. The G9-06 run itself is the maintainer's and was not performed.

**State found.** HEAD 67cfd88 on branch g9-report, clean tree apart from the
maintainer's ad hoc re-recorder `.local_rerecord_stale_reference.py` (read as
the starting point, not committed). The gate file held 45 VERIFIED tasks, two
FAILED (G3-05, G3-08) and the rest NOT_RUN; the "GPUs actually exercised" row
of RELEASE_SCOPE.md still read `NOT_RUN (G4-01 to G4-05)` while the gate file
had all six G4 tasks VERIFIED. Evidence commands carry their environment in
two spellings, `$env:NAME='1'; ...` and `NAME=1 ...`, and the reference script
read the scope from a key (`scope`) the recorder never writes.

**Changed files (why).**
- `scripts/build_validation_report.py`, `docs/VALIDATION_REPORT.md`, `docs/validation/known_limitations.json`, `tests/test_validation_report.py`, `docs/validation/cases/G9-07.json` (G9-07): the renderer, its output, the limitations list it prints, the tests and the pre-declared case.
- `docs/RELEASE_SCOPE.md`: the 24 "Verified for release" cells that start with a state name and a new marker-delimited stage-status table are regenerated by the renderer from the gate file; the prose is kept and the intro says so.
- `scripts/rerecord_gates.py`, `tests/test_rerecord_gates.py`, `docs/RELEASE_PROCEDURE.md` (G9-06 tooling): the re-recorder (selection, environment-prefix parsing in both spellings, dirty-tree refusal, `--wheel` with a fresh `.local/venvs/rc`, an installed-package import probe and absolute test paths), its tests on a temporary repository with a tiny pytest module, and the ordered release-candidate commands.
- `scripts/record_gate_evidence.py`: `--interpreter` records the Python, torch and CuPy versions of the interpreter that ran the command (the wheel environment) instead of the recorder's own; `--platform` writes the host's platform id (a record under docs/validation/platforms must exist) as `platform_id`, and `rerecord_gates.py --platform` forwards it. Older evidence without the field is assigned to a platform by its GPU names in the report.
- `docs/THIRD_PARTY_NOTICES.md`, `docs/validation/sbom.json`: regenerated with `scripts/provenance_inventory.py` after the merge of main (file counts 1332 of 1351); the report's consistency section runs `--check` and prints its summary, and RELEASE_PROCEDURE.md step 2 regenerates both before the wheel is built.
- `docs/RELEASE_REVIEW.md`, `docs/CHANGELOG.md`, `docs/validation/completion_gates.json` (G9-07 IMPLEMENTED with code paths and required tests; G9-06 IN_PROGRESS with code paths, required tests and a note that the RC run is pending).

**Commands run (device: local Windows 11 host, CPU only with `CUDA_VISIBLE_DEVICES=-1` for the test runs, the RTX 3060 left to the other agents; TMP and TEMP set to D:\TorchFDTD\.local\tmp).**

```
D:/TorchFDTD/.venv/Scripts/python.exe scripts/build_validation_report.py --allow-dirty      (development renders)
D:/TorchFDTD/.venv/Scripts/python.exe scripts/build_validation_report.py                    (on the committed tree)
D:/TorchFDTD/.venv/Scripts/python.exe scripts/build_validation_report.py --check
...
D:/TorchFDTD/.venv/Scripts/python.exe scripts/record_gate_evidence.py --task G9-07 --command "<the line above>" --junit D:/TorchFDTD/.local/tmp/junit/G9-07.xml --exit-code 0 --fixture docs/validation/cases/G9-07.json
D:/TorchFDTD/.venv/Scripts/python.exe scripts/check_release_gates.py --task G9-07
D:/TorchFDTD/.venv/Scripts/python.exe -m pytest -q -p no:cacheprovider tests/test_completion_program_documents.py tests/test_release_gates.py tests/test_compatibility_policy.py tests/test_readme_measurements.py tests/test_readme_meep_comparison.py tests/test_platform_matrix.py
```

**Measurements and pre-declared limits.** The case declares four
observables: the report equals a fresh render byte for byte, the scope cells
and stage block equal their regeneration, zero forbidden words, zero
consistency mismatches. On the committed tree the renderer reports 12
consistency checks and 0 mismatches: the version 0.14.0.dev0 agrees across
pyproject.toml, COMPATIBILITY.md, CHANGELOG.md and the clean-install wheel
name; the six README row tests of tests/test_readme_measurements.py pass when
called as functions; the README Meep block and MEEP_COMPARISON.md equal the
renderer output; `provenance_inventory.py --check` reports 52 components,
3 open items, 1332 files scanned, 0 findings, pip check exit 0 (the script
prints a UnicodeDecodeError from a cp949 reader thread of one of its
subprocesses on this host; its exit status and summary are unaffected and the
report reads the summary); 24 scope cells are rendered from the gate file. One
finding was corrected by the render rather than by hand: the "GPUs actually
exercised" row now reads `VERIFIED (G4-01, G4-02, G4-03, G4-04, G4-05)`.
The report's judge columns say what check_release_gates.py says: WORKSTATION
45 pass and 32 fail of 77 required tasks, HPC 45 pass and 38 fail of 83,
both NOT RELEASABLE; G3-05 and G3-08 keep their FAILED findings in the
physics table (G3-05 scattering 1.38, 0.458, 0.445 against budgets 0.5, 0.3,
0.2; G3-08 layer A 1.15e-4 against rtol 1e-4). A full render takes about
15 s (the 83 judgements are computed once per render).

**Passed / failed / skipped / not run.** tests/test_rerecord_gates.py: 9
passed, 0 failed, 0 skipped (with tests/test_release_gates.py, unchanged, 32
passed in 139.0 s; real pytest subprocesses on the temporary repository).
tests/test_validation_report.py: 8 passed, 0 failed, 0 skipped on the
committed tree (the evidence run). The neighbouring document and gate
...
- The G3 fixture commands with `TORCHFDTD_G3_RECORD=docs/validation/g3` rewrite the g3 records when replayed; the replayed evidence then carries a dirty manifest until those records are committed (documented in RELEASE_PROCEDURE.md step 3).
