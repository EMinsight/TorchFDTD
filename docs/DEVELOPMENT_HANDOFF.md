...
- `scripts/record_gate_evidence.py`: `--interpreter` records the Python, torch and CuPy versions of the interpreter that ran the command (the wheel environment) instead of the recorder's own.
...
consistency mismatches. On the committed tree the renderer reports 11
...
renderer output; 24 scope cells are rendered from the gate file. One
...
D:/TorchFDTD/.venv/Scripts/python.exe scripts/clean_install_check.py --local-root D:/TorchFDTD/.local --find-links D:/TorchFDTD/.local/wheels --cuda-torch "torch==2.10.0+cu126"
D:/TorchFDTD/.venv/Scripts/python.exe scripts/rerecord_gates.py --all --wheel D:/TorchFDTD/.local/dist/<commit12>/torchfdtd-0.14.0.dev0-py3-none-any.whl --torch "torch==2.10.0+cu126" --find-links D:/TorchFDTD/.local/wheels
```
