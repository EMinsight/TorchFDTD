...
- docs/PHYSICS_VALIDATION.md: branch g3-fixtures-a generates the whole file with `scripts/render_physics_validation.py` (a `SECTIONS` dict of task id to a function returning markdown lines), so the add/add conflict at merge is resolved by taking that branch's file, inserting `sys.path.insert(0, str(ROOT))` and `from benchmarks.render_g3_b import RENDERERS as G3_B_SECTIONS` into that script, merging `G3_B_SECTIONS` into `SECTIONS` and rerunning it; `benchmarks/render_g3_b.py` keeps its own marker-based writer only for this branch before the merge. The fine-mesh gate is `TORCHFDTD_G3_FULL` here and `TORCHFDTD_G3_FINE` on g3-fixtures-a; renaming after the recorded runs would change the test source and stale the four evidence runs, so both names stay.
