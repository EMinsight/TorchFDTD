...
| Size guards | `DefaultGuards` middleware in `torchfdtd/server.py`, `torchfdtd.models.default_guards` | Every request, body validation included, runs inside `default_guards()`. `Project.limits` and `Region.resident_cell_limit` let the Python API raise the caps on structures (1000), frequency-plane samples (12 million complex samples per plane) and resident cells (8 million); on the server a carried value above the default, or `null` (no cap), becomes the default and a lower value is kept. A request above a default cap answers 422 whatever limits it carries, and `/api/validate` echoes the project with the server's caps |
