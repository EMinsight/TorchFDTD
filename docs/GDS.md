...
`GDSLimits` bounds file bytes, hierarchy depth, expanded cell/geometry/label instances, total vertices, final structures and physical coordinates. The default maximum of 1000 converted structures is an import limit; the native project has no structure cap on the Python API, and an explicit larger `max_structures` is accepted. `max_total_vertices` defaults to `None`, no vertex limit, on the Python API; the workbench upload route passes 1,000,000 (`gds_service.SERVER_GDS_LIMITS`, [SECURITY.md](SECURITY.md)). Reference expansion is counted before flattening. Optional `xy_bounds_um=(xmin, ymin, xmax, ymax)` rejects geometry or port apertures outside a specified physical box. These admission checks do not automatically fit a simulation mesh or guarantee that a selected resolution fits memory.
...
Imported objects use the ordinary native CAD editor, Save project and Python export. The three projected views and the perspective view draw polygon holes from the same tessellation as the outer contour, and the vertex editor lists each hole as its own block after a blank line; edits are validated by the same native polygon rules. The Export GDS button of the Design ribbon writes the enabled rectangles and polygons to one cell through `POST /api/gds/export` (the same `export_gds` with an explicit layer/datatype per structure and a cell name) and downloads the `.gds` file together with its `.gds.json` layer-stack sidecar; circles, rings, spheres and tilted extrusions must be disabled first, as in Python. Optional dependency errors include the installation command. Uploaded files are stored under opaque IDs in this local server's result directory, and IDs are valid only in the current server session.

`tests/test_project_versioning.py` checks the export route (file bytes, sidecar, rejections, temporary file removal) and `tests/ui/g8-journey.spec.js` exports from the browser. `tests/test_gds_api.py` compares browser API conversion with direct Python import, checks that an `etch_by` layer row returns a project whose polygon carries its hole through validation and Python export, and validates optional-dependency/upload errors. `tests/ui/gds.spec.js` generates an independent synthetic GDS with an etch pair and exercises actual upload, explicit stack with the Etch by field and its rejection of a malformed pair, report hole counts, native CAD with the holed polygon, the hole block in the vertex editor, project save and Python download. Set `TORCHFDTD_TEST_PYTHON` to a Python interpreter with gdstk when running that UI test.
