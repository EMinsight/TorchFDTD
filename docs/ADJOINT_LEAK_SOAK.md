...
- Follow-up probe (not part of the 150-iteration record): DispersiveSimulation on the smallest CUDA fixture (20 x 16 cells, 30 steps, checkpoints 2) for 400 iterations in a fresh process: torch allocated 0.01 MiB and reserved 2.0 MiB constant; RSS 1155.30 (iteration 20) to 1156.07 MiB (iteration 400), of which 0.68 MiB is one step at iteration 60; private bytes 2873.1 to 2905.4 MiB by one 31.9 MiB step between iterations 220 and 230 (about 100 s after CUDA initialisation), then flat for 170 iterations. The step is once per process, RSS-neutral and independent of the path's device footprint (2 MiB reserved here, 44 MiB in tensor_project_cuda); it did not occur in a 200 s pure-torch CUDA loop without CuPy. Its owner was not identified with a native tracer; no Python, torch or CuPy allocator moves.
