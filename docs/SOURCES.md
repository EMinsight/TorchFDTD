...
The same preview (`torchfdtd.preview_source(project, source_id)`, `POST /api/sources/{id}/preview`) also reports what the solver realizes in space and what the waveform covers: the polarization vector of the E or H family; per injected component the source cells, their physical positions, the uniform amplitude and, on a Bloch cell, the spatial phase `exp(+i phi (x - x0) / L)` along each Bloch axis (a **Spatial phase** tab plots it); the effective bandwidth, declared as the band where the single-sided amplitude spectrum exceeds 1 percent of its peak (-40 dB) on the DFT bins of the whole simulation interval, with the declared wavelength range of a broadband source next to it; and the incidence definition. Every oblique source of the solver is a fixed-k_parallel source: the Bloch phase fixes `k_parallel = phi / L`, so the incidence angle `asin(k_parallel / (n k0))` varies across the band and the preview lists it at the carrier and at the band edges (an edge with `k_parallel > n k0` is named evanescent). A fixed-angle broadband source (the vendor's BFAST) has no code path: the **incidence definition** selector of the dialog asks the server for it and shows the registry's refusal (`torchfdtd.capabilities.INCIDENCE`, served in `/api/capabilities`) instead of drawing the source as if it were supported.
