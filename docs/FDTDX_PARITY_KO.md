...
- [열린 경계 mode port](OPEN_MODE_PORTS.md): 실제 native CPML 점화식의
  ...
  일반 branch/UI·streamed 지원의 완료로 표시하지 않는다.
