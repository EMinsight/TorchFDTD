...
- PEC and PMC/symmetric walls at the mesh endpoints, with point sources and monitors on the stored upper PMC faces and edges, and multipole ADE banks on the face nodes a dispersive material owns. Those cohorts reject TFSF/one-way sources and field monitors, and keep the stored face values in the NPZ as `endpoint_E_upper`/`endpoint_H_upper`.
