...
files += [Path(p) for p in ['README.md','LICENSE','THIRD_PARTY_NOTICES.txt','pyproject.toml','package.json','package-lock.json','vite.config.js','playwright.config.js','.gitignore']]
version=re.search(r'^version = "([^"]+)"',Path('pyproject.toml').read_text(),re.M)[1]
out=Path(f'dist/photonweave-{version}-source.zip');out.parent.mkdir(exist_ok=True)
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as z:
    for path in files:
        z.write(path,'photonweave/'+path.as_posix())
