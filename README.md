...
| Resident ([docs](docs/EXECUTION_MODES.md#memory-modes)) | The estimate fits 75% of free VRAM (80% of RAM on CPU); the workbench server also limits it to 8 million cells unless started with `--memory-admission` | The fastest path, live frames | The whole grid in one memory |
