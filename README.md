...
**Torch differentiation and memory:** an experimental `DifferentiableSimulation` connects real nondispersive epsilon and regularized sphere parameters to point signals, `loss.backward()` and Adam. Native CUDA forward and backward use a discrete Yee/CPML adjoint with bounded replay checkpoints and optional asynchronous host/disk staging. [`StreamedSimulation`](docs/STREAMED_FDTD.md) keeps global state in DRAM and transposes space-time slab dependencies, with reusable buffers and optional asynchronous transfers. `tune_streamed` selects admitted policies using two measured durations and checkpoint-replay costs. Explicit streamed scenes can exceed the resident cell cap, with an [explicit-budget 512³, 16-step forward/backward check](docs/validation/COMPACT_HOST_REPORT.md). Physical VRAM-overflow validation, unified memory policy, all-physics differentiation and port-normalized design remain pending. [API and limits](docs/DIFFERENTIABLE_FDTD.md), [complete example](examples/differentiable_design.py), [measured development results](docs/validation/ADJOINT_REPORT.md), [hierarchy plan](docs/HIERARCHICAL_EXECUTION.md).
